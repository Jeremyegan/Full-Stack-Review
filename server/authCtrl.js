...
module.exports = {
    register: async (req, res) => {
        ...
        const accountArr = await db.find_acc_by_email([email]) //array way of asking for the selection from the db. this is initialize in sql file by $1 which is indicating the first defined in the array... or in this case "email".
        if (accountArr[0]) {
        ...
        req.session.user = {email: accountArr[0].acc_email, id: newAccArr[0].acc_id};
        res.status(200).send({
            message: 'Logged in.',
